...
def create_ovs_vif_port(bridge,dev_name,root_helper):
    args=['ovs-vsctl', '--', '--may-exist', 'add-port',
          bridge, dev_name,
          '--', 'set', 'Interface', dev_name,
          'external-ids:iface-status=active'] 
    try:
        utils.execute(args, root_helper=root_helper)        
    except Exception, e:
        LOG.error(_("Unable to execute %(cmd)s. Exception: %(exception)s"),
                   {'cmd': args, 'exception': e})  
...
def _delete_veth_pair(dev1_name, dev2_name,root_helper):
    #for dev_name in [dev1_name,dev2_name]:
    try:
        utils.execute(['ip','link','delete',dev1_name], root_helper=root_helper)
    except Exception, e:
        LOG.error(_("Unable to execute %(cmd)s. Exception: %(exception)s"),
                  {'cmd': args, 'exception': e})        
            
        
def _create_veth_pair(dev1_name, dev2_name, root_helper):
    args = ['ip', 'link', 'add', dev1_name, 'type', 'veth', 'peer','name', dev2_name]
    try:
        utils.execute(args, root_helper=root_helper)
    except Exception, e:
        LOG.error(_("Unable to execute %(cmd)s. Exception: %(exception)s"),
    ...
    for dev in [dev1_name, dev2_name]:         
        utils.execute(['ip', 'link', 'set', dev, 'up'], root_helper=root_helper)
        utils.execute(['ip', 'link', 'set', dev, 'promisc', 'on'],root_helper=root_helper)  
