...
class IptablesFirewallDriver(firewall.FirewallDriver):
    ...
    def _add_fallback_chain_v4v6(self):
        ...
        #self.iptables.ipv4['filter'].add_rule('sg-fallback', '-j ACCEPT')        
        self.iptables.ipv6['filter'].add_chain('sg-fallback')
        self.iptables.ipv6['filter'].add_rule('sg-fallback', '-j DROP')
        #self.iptables.ipv6['filter'].add_rule('sg-fallback', '-j ACCEPT')
    ...
    def _ban_ipv6_dad(self):
        return ['-s ::/128 -p icmpv6 -j DROP']    
    ...
    def _add_rule_by_security_group(self, port, direction):
        ...
        LOG.debug(_("value ipv6_sg_rules is (%s)"), ipv6_sg_rules)    
        ipv4_iptables_rule = []
        ipv6_iptables_rule = []
        #lee to enable to change ip address mannuly
        if direction == EGRESS_DIRECTION:
            ipv4_iptables_rule += self._arp_spoofing_rule(port)
            ipv6_iptables_rule += self._arp_spoofing_rule(port)
            ipv6_iptables_rule += self._ban_ipv6_dad()
