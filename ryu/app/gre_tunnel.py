...
def cls_rule(in_port=None, tun_id=None, dl_type=None, dl_src=None, 
             dl_dst=None, ipv6_dst=None,ipv6_dst_mask=None):
    ...
    if dl_type == 0x86dd and ipv6_dst is not None and ipv6_dst_mask is not None:
        rule.set_ipv6_dst_masked(ipv6_dst, ipv6_dst_mask)
    return rule
...
class GRETunnel(app_manager.RyuApp):
    ...
    def _vm_port_add(self, ev):
        ...
        ipv6_rule = cls_rule(tun_id=tunnel_key, dl_type=0x86dd,ipv6_dst=[0xff00,0,0,0,0,0,0,0],ipv6_dst_mask=[0xff00,0,0,0,0,0,0,0])
        actions = []
        ...
        self.send_flow_mod(dp, ipv6_rule, self.LOCAL_OUT_TABLE, command,
                           self.LOCAL_OUT_PRI_BROADCAST, actions)
